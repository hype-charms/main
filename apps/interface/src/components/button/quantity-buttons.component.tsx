...
export const QuantityButtonsComponent: FC<QuantityButtonsComponentProps> = ({ productId, onUpdateQuantities, quantity, cartHasItem, disabled }) => {
    ...
    return <div className="flex flex-row items-center justify-center gap-2">
        <button
            ...
            aria-label={disabled ? "disabled" : quantities === 1 ? "quantity can't be less than 1" : "decrease item quantity"}
            tabIndex={0}
            className={`${disabled || cartHasItem || quantities === 1 ? "opacity-50" : "opacity-100"} h-6 w-6 text-2xl flex`}
            disabled={disabled || cartHasItem || quantities === 1}
        ...
        <p
            aria-label={`you have ${quantities} of these in your cart`}
            className={`${disabled ? "opacity-50" : "opacity-100"} text-xl flex justify-center items-center text-center w-6`}>
            {quantities}
        </p>
        <button
            type="button"
            name="decrease item quantity"
            aria-label={disabled ? "disabled" : quantities === 10 ? "quantity can't be more than 10" : "decrease item quantity"}
            tabIndex={0}
            className={`${disabled || cartHasItem || quantities === 10 ? "opacity-50" : "opacity-100"} h-6 w-6 text-2xl flex`}
            disabled={disabled || cartHasItem || quantities === 10}
